...
-- Tabla para rastrear archivos procesados y evitar gasto de tokens
create table if not exists import_logs (
  id bigint generated by default as identity primary key,
  user_id uuid references auth.users on delete cascade not null,
  file_hash text not null, -- Huella digital única del archivo
...
create policy "Users can insert their own logs" on import_logs for insert with check (auth.uid() = user_id);
create policy "Users can read their own logs" on import_logs for select using (auth.uid() = user_id);