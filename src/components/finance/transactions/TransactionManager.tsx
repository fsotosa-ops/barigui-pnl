...
export const TransactionManager = ({ transactions, setTransactions, onAdd, onDelete }: TransactionManagerProps) => {
  ...
  const financialAnalysis = useMemo(() => {
    ...
    return { totalIncome, totalExpense, netBalance: totalIncome - totalExpense, categoryStats: Object.values(categories) };
  }, [filteredTxs]);

  const formatMoney = (val: number) => {
    const rate = rates[localCurrency] || 1;
    return (val * rate).toLocaleString('en-US', { style: 'currency', currency: localCurrency, maximumFractionDigits: 0 });
  };

  return (
    <div className="flex flex-col gap-6 animate-in fade-in duration-500 pb-20">
      {/* Selector de Ámbito Global */}
      <div className="flex justify-between items-center">
        ...
        <select value={localCurrency} onChange={(e) => setLocalCurrency(e.target.value)} className="bg-white border border-slate-200 rounded-xl px-3 py-2 text-xs font-black outline-none">
          {['USD', 'CLP', 'BRL', 'EUR'].map(c => <option key={c} value={c}>{c}</option>)}
        </select>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm flex items-center gap-4">
          ...
          <div>
            <p className="text-[10px] font-black text-slate-400 uppercase">Ingresos {activeScope !== 'all' && activeScope}</p>
        ...
        <div className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm flex items-center gap-4">
          ...
          <div>
            <p className="text-[10px] font-black text-slate-400 uppercase">Gastos {activeScope !== 'all' && activeScope}</p>
        ...
        <div className={`p-6 rounded-[2rem] border shadow-sm flex items-center gap-4 ${financialAnalysis.netBalance >= 0 ? 'bg-emerald-900 border-emerald-800' : 'bg-rose-900 border-rose-800'}`}>
          ...
          <div>
            <p className="text-[10px] font-black text-white/50 uppercase">Balance Neto</p>
      ...
      <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm p-8 min-h-[600px] flex flex-col">
        <div className="flex justify-between items-center mb-8">
          ...
          <button onClick={() => { setEditingItem(null); setIsModalOpen(true); }} className="bg-slate-900 text-white px-6 py-3 rounded-xl font-black text-xs flex items-center gap-2 hover:bg-black transition-all shadow-lg"><Plus size={16} /> Nueva Transacción</button>
        </div>
        ...
        {viewMode === 'list' ? (
          ...
          <div className="flex-1 flex items-center justify-center text-slate-400 italic">Visualización de insights optimizada para el ámbito {activeScope}...</div>
        )}
      </div>
